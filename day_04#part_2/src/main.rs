...
fn main() {
  ...
  let mut boards_success: Vec<bool> = Vec::with_capacity(boards.len());

  for _ in ro_boards.iter() {
    boards_success.push(false);
  }
  ...
  'find_winner: for num in drawn {
    for (b, board) in ro_boards.iter().enumerate() {
      ...
      if !boards_success[b]
        && (boards[b].iter().any(|&line| line.iter().all(|&x| x < 0))
            || (0..5).any(|i| boards[b].iter().all(|&line| line[i] < 0))) {
        boards_success[b] = true;

        if boards_success.iter().all(|&b| b) {
          let score = boards[b].iter().fold(0, |acc, x| acc + x.iter().fold(0, |acc, x| acc + if x > &0 { x } else { &0 }));

          println!("Winner is {}! Score is {} × {} = {}", b, score, num, score*num);
          println!("Board: {:?}", boards[b]);
